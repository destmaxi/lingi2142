...
is_hall_link_pyth_ok () {
  ...
  if [ $? -ne 0 ]
  then
          echo "ERREUR PING From 4 to fd00:300:f24::4"
          return 0
  fi
  ping6 -q -c1 fd00:300:4:f24::2 > /dev/null
  if [ $? -ne 0 ]
  then
          echo "ERREUR PING From 4 to fd00:300:f24::2"
          return 0
  fi
  return 1
}
...
if [ ${content} -eq 1 ] && [ $table -eq 0 ];then
  ...
  ip -6 rule add from fd00:200:4::/48 to all priority 2000 table 10

  a="$(ip -6 route show | grep fd00:300:4:f00::2 | cut -c23-60 | awk -F dev '{print $1}')"
  ip -6 route del default table 10
  ip -6 route add default via ${a} dev PYTH-eth0 table 10

  table=1
fi
if [ ${content} -eq 1 ];then
  into_table_10="$(ip -6 route ls table 10 | cut -c13-60 | awk -F dev '{print $1}')"
  into_routing_table="$(ip -6 route show | grep fd00:300:4:f00::2 | cut -c23-60 | awk -F dev '{print $1}')"
  if [ ${into_routing_table} != ${into_table_10} ];then
    ip -6 route del default table 10
    ip -6 route add default via ${into_routing_table} dev PYTH-eth0 table 10
  fi
fi
if [ ${content} -eq 0 ] && [ $table -eq 1 ];then
  echo "link down"

  a="$(ip -6 route show | grep fd00:300:4:f00::1 | cut -c23-60 | awk -F dev '{print $1}')"

  ip -6 route del default dev PYTH-eth0 table 10
  ip -6 route add default via ${a} dev PYTH-eth1 table 10
