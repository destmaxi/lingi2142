...
is_hall_link_pyth_ok () {
  ...
  if [ $? -ne 0 ]
  then
          echo "ERREUR PING From 5 to fd00:300:f24::4"
          return 0
  fi
  ping6 -q -c1 fd00:300:4:f24::2 > /dev/null
  if [ $? -ne 0 ]
  then
          echo "ERREUR PING From 5 to fd00:300:f24::2"
          return 0
  fi
  return 1
}
...
sleep 5 #we wait 5sec during each loop to try not to overuse the network power
    ...
    if [ ${content} -eq 0 ] && [ $table -eq 0 ];then
     ...
     b="$(ip -6 route show | grep fd00:300:4:f00::3 | cut -c23-60 | awk -F dev '{print $1}')"

     ip -6 route add default via ${a} dev SH1C-eth1 table 20
     ip -6 route add default via ${b} dev SH1C-eth0 table 30
